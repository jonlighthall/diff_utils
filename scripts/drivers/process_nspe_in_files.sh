    ...
    if $model_blank || (grep -qi '^tl' "$infile" || grep -qi '^rtl' "$infile" || grep -Eiq '^hrfa|^hfra|^hari' "$infile"); then
        ...
        if ($skip_existing || $skip_newer) && ! $force; then
            ...
            if $skip_existing; then
                # For make/test modes: only skip if primary outputs exist
                ...
                if [[ "$mode" == "make" || "$mode" == "test" ]]; then
                    ...
                    # For copy/diff: check both primary and secondary files
                    existing_count=0
                    for output in "${primary_outputs[@]}" "${secondary_outputs[@]}"; do
                        if [[ -f "$output" ]]; then
                            ((existing_count++))
                        fi
                    done
