...
if [[ ! "$diff_level" =~ ^[0-3]$ ]]; then
    ...
    echo "  1 - diff only (no fallback to tldiff or tl_diff)"
    echo "  2 - max tldiff (stop at tldiff, don't try tl_diff)"
    echo "  3 - force tl_diff (always run all three diff tools)"
    exit 1
fi
...
if [[ -n "$test_dir" || -n "$ref_dir" ]]; then
    ...
    for bn in "${basenames[@]}"; do
        ...
        for suffix in 01 02 03; do
            ...
            parse_diff_output "$diff_output_file"
            ...
            if [[ $diff_exit_code -eq 0 ]]; then
                ...
                printf '%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n' \
                    ...
                    "${DIFF_MAX_ERROR:--}" "${DIFF_N_SIG:--}" "${DIFF_N_NZ:--}" "${DIFF_RMSE:--}" "${DIFF_RMSE_MAX:--}" "${DIFF_M1:--}" \
                    >> "$DIFF_LOG"
                ...
                if [[ -f "$test" && -s "$test" ]]; then
                    if [[ "$mode" == "copy" ]]; then
                        ...
                        if [[ ! -f "$ref" ]]; then
                            ...
                            echo -e "\n   \e[36m[[INFO]]\e[0m $(basename "$test") exists but $(basename "$ref") not found, checking other suffixes..."
        ...
        if [[ "$found_files" == false ]]; then
            ...
            if [[ "$mode" == "copy" ]]; then
                if $any_output_exists && ! $all_outputs_empty; then
                    echo -e "\n   \e[33m[[INFO]]\e[0m Output files exist but were not copied (no reference check in copy mode)"
                else
                ...
                echo -e "\n   \e[36m[[INFO]]\e[0m Output files will be generated by ${PROG} for suffixes (01, 02, 03)"
            else
