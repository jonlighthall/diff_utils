...
for infile in "${infiles[@]}"; do
    ...
    if ($skip_existing || $skip_newer) && ! $force; then
        ...
        if $skip_existing; then
            # For make/test modes: skip execution if primary output exists
            if [[ "$mode" == "make" || "$mode" == "test" ]]; then
                if [[ -f "$primary_output" ]]; then
                    skip_execution="output file exists"
                ...
                # For copy mode: skip entirely if .tl reference already exists
                ref="$directory/${basename_noext}.tl"
                if [[ -f "$ref" ]]; then
                    skip_entirely="reference file exists"
                fi
            fi
        fi
        ...
        if $skip_newer && [[ -z "$skip_execution" ]]; then
            ...
            if [[ -f "$primary_output" && "$primary_output" -nt "$infile" ]]; then
                skip_execution="output newer than input"
                if [[ "$mode" == "make" ]]; then
                    skip_entirely="$skip_execution"
                fi
            fi
    ...
    if [[ ("$mode" == "test") ]] || [[ "$mode" == "copy" && -f "$test" && -s "$test" ]] || [[ "$mode" == "diff" && (-f "$test" || -f "$ref") ]]; then
        ...
        if [[ "$mode" == "copy" ]]; then
            ...
            # In test mode, check if reference exists to determine if this is an error
            if [[ -f "$parent_dir/${basename_noext}.tl" ]]; then
                # Reference exists but no output - this is a REAL ERROR
                ...
                # No reference and no output - just info, not an error
                echo -e "   \e[36m[[INFO]]\e[0m Output file tl.line will be generated by ${PROG} in parent directory"
                echo -e "   \e[33mHint: Run 'copy' mode to establish reference file after first successful run\e[0m"
            fi
        else
