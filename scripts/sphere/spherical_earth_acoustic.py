...
def calculate_ray_depth_profile(source_depth, beam_angle, r_source, range_m, central_angle, r_earth, num_points=100):
    """
...
def plot_geometry(source_depth, beam_angle, r_source, range_m, central_angle, r_earth):
    ...
    source_x = r_source * np.cos(source_angle)
    source_y = r_source * np.sin(source_angle)
    ax.plot(source_x/1000, source_y/1000, 'go', markersize=10, label=f'Source ({source_depth:.0f} m depth)')
    ...
    if range_m > 0:
        ...
        intersect_x = r_earth * np.cos(intersect_angle)
        intersect_y = r_earth * np.sin(intersect_angle)
        ax.plot(intersect_x/1000, intersect_y/1000, 'ro', markersize=10,
                label=f'Intersection ({range_m/1000:.1f} km range)')

        # Draw the ray path
        ax.plot([source_x/1000, intersect_x/1000],
                [source_y/1000, intersect_y/1000],
                'r--', linewidth=2, label='Acoustic ray')

        # Draw radii to source and intersection
        ax.plot([0, source_x/1000], [0, source_y/1000], 'k--', alpha=0.3, linewidth=1)
        ax.plot([0, intersect_x/1000], [0, intersect_y/1000], 'k--', alpha=0.3, linewidth=1)
        ...
        ax.text(text_x/1000, text_y/1000, f'{central_angle*RAD_TO_DEG:.2f}°',
    ...
    ax.set_title('Spherical Earth Acoustic Ray Geometry\n(Full View)', fontsize=14, weight='bold')
    ...
    if range_m > 0:
        ...
        depth_surface = r_earth - y_surface_cart  # Depth of surface points below the top
        ...
        ax.fill_between(x_surface/1000, depth_surface,
                        np.interp(x_surface, chord_x, chord_depth),
                        where=(depth_surface <= np.interp(x_surface, chord_x, chord_depth)),
                        color='cyan', alpha=0.2, label='Water column', zorder=1)

        # Plot the curved Earth surface (blue solid)
        ax.plot(x_surface/1000, depth_surface, 'b-', linewidth=2,
                label='Ocean surface (curved)', zorder=2)

        # Plot the straight acoustic ray path (red dashed - horizontal line)
        ax.plot(chord_x/1000, chord_depth,
                'r--', linewidth=2.5, label='Acoustic ray (straight)', zorder=3)

        # Mark source (green) and intersection (red)
        ax.plot(x_source/1000, depth_source, 'go', markersize=10,
                label=f'Source ({source_depth:.0f} m depth)', zorder=5)
        ax.plot(x_intersect/1000, depth_intersect, 'ro', markersize=10,
                label='Surface intersection', zorder=5)

        # Add annotations
        ax.text(x_intersect/2000, depth_surface.max()*0.2,
                f'Surface curves down\n{range_m/1000:.2f} km arc', fontsize=9, ha='center',
                bbox=dict(boxstyle='round,pad=0.3', facecolor='lightblue', alpha=0.8))

        # Calculate chord length
        chord_length = np.sqrt((x_intersect_cart - x_source_cart)**2 + (y_intersect_cart - y_source_cart)**2)
        ax.text(x_intersect/2000, depth_source*0.5,
                f'Ray stays straight\n{chord_length/1000:.2f} km chord', fontsize=9, ha='center',
    ...
    ax.set_title('Physical Geometry: Straight Ray vs Curved Earth', fontsize=14, weight='bold')
    ...
    ax.axhline(y=0, color='b', linewidth=2, label='Ocean surface (flat reference)', zorder=2)

    # Plot the CURVED acoustic ray path (red dashed)
    ax.plot(horiz_ranges/1000, depths, 'r--', linewidth=2.5, label='Acoustic ray path', zorder=3)

    # Mark max depth point
    ax.plot(max_depth_range/1000, max_depth, 'mo', markersize=8,
            label=f'Max depth: {max_depth:.1f} m at {max_depth_range/1000:.1f} km', zorder=4)

    # Mark source and intersection points (swapped colors: green=source, red=intersection)
    ax.plot(0, source_depth, 'go', markersize=10, label=f'Source: {source_depth:.0f} m', zorder=5)
    ax.plot(range_m/1000, 0, 'ro', markersize=10, label='Surface intersection', zorder=5)
    ...
    ax.set_ylabel('Depth Below Surface (m)', fontsize=12)
    ax.set_title('Depth Profile: Ray Curves Up to Meet Flat Surface', fontsize=14, weight='bold')
    ax.legend(loc='best', fontsize=9)
...
def main():
    ...
    if args.angle is not None:
        ...
        beam_angle = float(input("Enter beam angle from horizontal in degrees\n"
    ...
    if range_m < 0:
        ...
        if abs(beam_angle) > 1e-6:
            ...
            if flat_earth_range > 0.0:
                ...
                print(f"Curvature effect:     {(range_km - flat_earth_range/1000):8.2f} km difference")
    ...
    if range_m > 0:
        try:
            ...
            try:
                input("Press Enter to close the plot and exit (or close the plot window manually)...")
