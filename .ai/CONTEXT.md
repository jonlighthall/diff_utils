...
**Goal:** Validate that `uband_diff`'s sub-LSB detection works correctly across files generated by different programming languages, which may use different rounding methods when formatting floating-point output.
...
2. Run `uband_diff` on cross-language comparisons
