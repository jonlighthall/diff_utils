...
// We craft lines where raw differences exist but rounding to the shared
// precision zeroes them out. Example: values differ in the 3rd decimal place
// but both have only 2 decimals -> rounded diff == 0 -> trivial. We then run
// with user_significant=0.0 (max sensitivity) and with a small positive
// threshold to verify trivial counts are identical and never migrate into
// significant/insignificant splits.
TEST(TrivialExclusionTest, TrivialDifferencesRemainExcludedAtZeroThreshold) {
  // Each line: col1 values differ in 3rd decimal place (0.001), printed
  // precision intentionally limited. Format ensures both sides present 2
  // decimal places so rounding eliminates the difference.
  std::string ref =
      "10.12 20.45 30.78\n"
      "11.34 21.56 31.67\n"
      "12.90 22.10 32.55\n";  // 12.90 vs 12.901 would have been non-trivial if
                              // precision higher
  std::string test =
  ...
  // Thresholds: significant=0.0 triggers sensitive semantics; critical large;
  // print small to attempt emission. Construct comparators directly using
  // public ctor signature used elsewhere in tests
  FileComparator cmp_sensitive(/*user_thresh*/ 0.0, /*hard*/ 9999.0,
                               /*print*/ 0.0, /*debug*/ 0);
  FileComparator cmp_normal(/*user_thresh*/ 0.05, /*hard*/ 9999.0,
                            /*print*/ 0.0, /*debug*/ 0);
  ...
  // elements and diff_non_trivial = 0 in both modes.
  ...
  EXPECT_EQ(stats_sensitive.diff_non_trivial, 0)
      << "Sensitive mode incorrectly treated formatting noise as non-trivial";
  EXPECT_EQ(stats_normal.diff_non_trivial, 0)
      << "Normal mode incorrectly treated formatting noise as non-trivial";

  EXPECT_EQ(stats_sensitive.diff_trivial, (size_t)total_elements);
  EXPECT_EQ(stats_normal.diff_trivial, (size_t)total_elements);

  // No significant or insignificant counts should appear (they partition only
  // non_trivial subset)
