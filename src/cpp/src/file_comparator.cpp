...
bool FileComparator::compare_files(const std::string& file1,
  ...
  if (verbosity.level > 0 || debug.enabled) {
...
double FileComparator::calculate_threshold(int ndp) {
  ...
  if (flag.new_fmt && debug.enabled && !verbosity.quiet) {
  ...
  if (thresh.significant < dp_threshold) {
    if (debug.enabled && flag.new_fmt && thresh.significant > 0) {
...
bool FileComparator::process_difference(const ColumnValues& column_data,
  ...
  double effective_print_thresh =
      (table.threshold == 0.0) ? thresh.zero : table.threshold;
  ...
  bool print_all_nonzero = (table.threshold == 0.0);
  ...
  if (diff_for_threshold > effective_print_thresh) {
    if (diff_is_zero && !debug.enabled && !debug.detailed && !debug.verbose) {
...
void FileComparator::print_table(const ColumnValues& column_data,
  ...
  // analyzing silently.
  if (flag.has_critical_diff || counter.diff_print >= table.max_rows) {
...
void FileComparator::print_rounded_summary(const SummaryParams& params) const {
  ...
  if (counter.diff_significant > 0 && !verbosity.show_statistics) {
    return;
  }
...
void FileComparator::print_significant_differences_printing_status(
  ...
  print_count_with_percent(
      params,
      "Printed differences       ( >" + std::to_string(table.threshold) + ")",
      counter.diff_print);
  ...
  if (counter.elem_number > 0) {
    ...
    std::cout << "      has_printed_diff    : "
              << format_boolean_status(flag.has_printed_diff, true, true,
                                       table.threshold <= thresh.significant)
              << std::endl;
    std::cout << "   new_fmt: "
...
void FileComparator::print_additional_diff_info(
  ...
  if (counter.diff_print == 0) {
    ...
    if (table.threshold > thresh.significant && counter.diff_significant > 0) {
      std::cout << "\033[1;33m   Warning: All significant differences are "
                   "below the print threshold ("
                << table.threshold << ")\033[0m" << std::endl;
    } else if (counter.diff_significant == 0) {
      std::cout << "\033[1;32m   Files " << params.file1 << " and "
                << params.file2
...
void FileComparator::print_summary(const std::string& file1,
  ...
  if ((!verbosity.quiet || debug.enabled || flag.error_found) &&
      (verbosity.level >= 0 || counter.diff_non_zero > 0)) {
    std::cout << "SUMMARY:" << std::endl;
  }
...
void FileComparator::print_settings(const std::string& file1,
  ...
  if (debug.enabled || verbosity.show_statistics) {
    std::cout << "   Debug mode : " << (debug.enabled ? "ON" : "OFF")
              << std::endl;
    std::cout << "   Debug level: " << verbosity.level << std::endl;
    std::cout << "   Print mode : " << (verbosity.quiet ? "DIFF" : "FULL")
              << std::endl;
    std::cout << "   File1: " << file1 << std::endl;
    std::cout << "   File2: " << file2 << std::endl;
  }
