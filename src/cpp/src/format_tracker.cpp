...
bool FormatTracker::validate_and_track_column_format(
    ...
    size_t& prev_n_col, size_t line_number, Flags& flags) {
  // Check if both lines have the same number of columns
  if (n_col1 != n_col2) {
    std::cerr << "Line " << line_number << " has different number of columns!"
              << std::endl;
    return false;
  }
...
bool FormatTracker::validate_decimal_column_size(
  ...
  if (dp_per_col.size() != column_index + 1) {
    std::cerr << "Warning: dp_per_col size mismatch at line " << line_number
              << std::endl;
...
bool FormatTracker::initialize_decimal_place_format(
...
double FormatTracker::calculate_threshold(int decimal_places,
