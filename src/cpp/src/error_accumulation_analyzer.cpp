 ...
 * @author J. Lighthall
 * @date February 2026
 * Adapted from reference/error_accumulation_analyzer.cpp
 */
...
AccumulationMetrics ErrorAccumulationAnalyzer::analyze(
  ...
  if (data.n_points < min_points_) {
    ...
    metrics.recommendation = "Action: Collect more comparison data points.";
    return metrics;
  }
...
ErrorAccumulationAnalyzer::linear_regression(const std::vector<double>& x,
  ...
  if (n > 2 && denominator != 0.0) {
    ...
    double z = std::abs(result.slope / result.std_error_slope);
...
// ============================================================================
// Interpretation and recommendation text
// ============================================================================
  ...
  switch (metrics.pattern) {
    case Pattern::SYSTEMATIC_GROWTH:
      oss << "Errors increase linearly with range at a rate of "
          ...
          << "  The high R^2 (" << std::fixed << std::setprecision(3)
    ...
    case Pattern::SYSTEMATIC_BIAS:
      oss << "Errors show a fixed offset (intercept = " << std::scientific
          << metrics.intercept << ")\n"
          << "  with negligible range dependence. R^2 = " << std::fixed
