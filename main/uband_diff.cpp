...
bool parse_threshold_argument(const char* arg, double& value, const std::string& name);
...
bool parse_file_arguments(int argc, char* argv[], ProgramArgs& args) {
  if (argc >= 3) {
    ...
    if (args.file1 == args.file2) {
      std::cerr << "\033[1;33mWARNING:\033[0m Both files have the same name: '" << args.file1 << "'" << std::endl;
    ...
    if (!test_file1.good()) {
      std::cerr << "\033[1;33mWARNING:\033[0m Cannot access first file: '" << args.file1 << "'" << std::endl;
    ...
    if (!test_file2.good()) {
      std::cerr << "\033[1;33mWARNING:\033[0m Cannot access second file: '" << args.file2 << "'" << std::endl;
      std::cerr << "         The file may not exist or is not readable." << std::endl;
      std::cerr << "         Will attempt to proceed (error will be reported by comparator)." << std::endl;
    }
  }
...
bool parse_threshold_argument(const char* arg, double& value, const std::string& name) {
  try {
    value = std::stof(arg);
    if (value < 0) {
      std::cerr << "\n\033[1;31mERROR:\033[0m " << name << " must be non-negative." << std::endl;
      std::cerr << "       Got: " << arg << " (parsed as " << value << ")" << std::endl;
      std::cerr << "       Valid range: [0, ∞)" << std::endl;
      return false;
    }
    return true;
  } catch (const std::invalid_argument&) {
    std::cerr << "\n\033[1;31mERROR:\033[0m Invalid " << name << " format." << std::endl;
    std::cerr << "       Expected: floating-point number (e.g., 0.05, 1.5, 10)" << std::endl;
    std::cerr << "       Got: '" << arg << "'" << std::endl;
    return false;
  } catch (const std::out_of_range&) {
    std::cerr << "\n\033[1;31mERROR:\033[0m " << name << " value out of range." << std::endl;
    std::cerr << "       Got: '" << arg << "'" << std::endl;
    return false;
  }
}
...
bool parse_numeric_arguments(int argc, char* argv[], ProgramArgs& args) {
  ...
  if (argc >= 4 && !parse_threshold_argument(argv[3], args.count_level, "Diff threshold")) {
    return false;
  }
  ...
  if (argc >= 5) {
    ...
    if (args.stop_level < args.count_level) {
      std::cerr << "\n\033[1;33mWARNING:\033[0m High threshold (" << args.stop_level
                << ") is less than diff threshold (" << args.count_level << ")." << std::endl;
      std::cerr << "         This may lead to unexpected behavior." << std::endl;
    }
  }

  if (argc >= 6 && !parse_threshold_argument(argv[5], args.print_level, "Print threshold")) {
    return false;
  }
...
int main(int argc, char* argv[]) {
  ...
  FileComparator comparator(args.count_level, args.stop_level, args.print_level, args.debug_level);
