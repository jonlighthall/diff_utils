...
TEST(SemanticInvariants, ZeroThresholdInvariant) {
  ...
  EXPECT_EQ(c.diff_significant + c.diff_high_ignore, c.diff_non_trivial)
      << "Zero-threshold semantic invariant failed: significant + high_ignore != non_trivial";
}
...
//    First difference non-critical & printable, second difference critical; table should stop after first line's row.
TEST(SemanticInvariants, CriticalSuppressionStopsPrinting) {
  writeFile("test_crit_sup1.txt", {"0.0", "0.0"});
  writeFile("test_crit_sup2.txt", {"0.5", "2.0"}); // second line is critical (>1.0)
  ...
  // Two elements compared, both non-zero & non-trivial => two significant diffs expected.
  EXPECT_EQ(c.diff_significant, 2u);
  // Only first (non-critical) printed; second suppressed after critical flag set.
  EXPECT_EQ(c.diff_print, 1u) << "Printing should stop after first critical encountered";
}

// 4. Print threshold decoupling:
//    Differences significant but below print threshold should not increment diff_print.
TEST(SemanticInvariants, PrintThresholdDecouplesCounting) {
  writeFile("test_print_decouple1.txt", {"0.000 0.000 0.000"});
  writeFile("test_print_decouple2.txt", {"0.200 0.150 0.120"}); // all diffs > user(0.05)
  FileComparator cmp(0.05, 5.0, /*print threshold*/1.0, 0); // print threshold higher than diffs
  cmp.compare_files("test_print_decouple1.txt", "test_print_decouple2.txt");
  const auto& c = cmp.getCountStats();
  EXPECT_GT(c.diff_significant, 0u);
  EXPECT_EQ(c.diff_print, 0u) << "No rows should print when below print threshold";
}
